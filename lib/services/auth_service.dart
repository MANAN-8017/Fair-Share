...
class AuthService {
  ...
  Future<String?> login(String identifier, String password) async {
    try {
      ...
      if (emailPattern.hasMatch(identifier)) {
        final valid = validateLogin(identifier, password, 1);
        if (valid != null) {
          return valid;
        }
      ...
      // PHONE LOGIN
      final valid = validateLogin(identifier, password, 0);
      if (valid != null) {
        return valid;
      }
  ...
  Future<String?> register(String name, String email, String phoneNumber, String password, String confirmPassword) async {
    final valid = validateRegistration(name, email, phoneNumber, password, confirmPassword);
    if (valid != null) {
      return valid;
    }
    try {
...
String? validateLogin(String identifier, String password, int x){
  if(x == 1) {
    return ValidationService.validate(identifier, type: ValidationType.email) ??
          ValidationService.validate(password, type: ValidationType.password);
  }
  return ValidationService.validate(identifier, type: ValidationType.phone) ??
        ValidationService.validate(password, type: ValidationType.password);
}